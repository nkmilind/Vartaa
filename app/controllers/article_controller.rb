class ArticleController < ApplicationController
    ...
    def index
        @article = Article.select(
                ...
                ' JOIN ranking on ranking.id = article.id'
            ).paginate(
                page: params['page']
            ).order(
                'ranking.admin', 'ranking.page_rank'
            ) 
    ...
    def show
        ...
        @comments = Comments.select(
                '"user".name as name, comments.comment as comment'
            ).where(
    ...
    def like
        if current_user
            u_likes = Userlikes.find_by(
                article_id: params["id"], user_id: current_user.id 
            )

            if u_likes.nil?  
                Userlikes.create(
                    user_id: current_user.id, likes: 1, article_id: params["id"]
                )  
            else
                Userlikes.where(
                    article_id: params["id"], user_id: current_user.id 
            ...
            likes = Userlikes.where(
                article_id: params["id"], likes: 1
            ).count()
    
            dislikes = Userlikes.where(
                article_id: params["id"], likes: 0
            ).count()
            
            Ranking.where(
                id: params["id"]
            ).update_all(likes: likes, dislikes: dislikes)

            redirect_to :back
        else
            flash[:notice1] = "Please sign in to upvote an article"
    ...
    def dislike
        if current_user
            ...
            if u_likes.nil?  
                Userlikes.create(
                    user_id: current_user.id, likes: 0, article_id: params["id"]
                )  
            else 
                Userlikes.where(
                    article_id: params["id"], user_id: current_user.id 
            ...
            Ranking.where(
                id: params["id"]
            ).update_all(dislikes: dislikes, likes: likes)
